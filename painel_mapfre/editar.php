...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    $sql_select = 'SELECT * FROM pmapfre where id = ' . $_GET['id'];
...
<body style="background-color:	#FF0000">
    <h2 style="text-align:center; color:#FFF">EDITAR PROCESSOS - MAPFRE SEGUROS</h2>
    ...
    <form action="editar.php?id=<?= $_GET['id'] ?>" method="post" enctype="multipart/form-data">
        ...
        <select name="status">
            ...
            <option value="cadastro" <?= ($status == 'cadastro') ? 'selected' : '' ?>>Cadastro</option>
